...
def delete(id):
  if request.method == "POST": # submitted the confirmation form
    # get student id from the form
    student_id = request.form.get("stu_id").upper()
    # now check to see if the student id from the form matches the one on the pass
    if student_id == q.get_student_id(id): 
      num_rows = q.delete_pass(id)
    else: # not a match
      num_rows = 0
    if num_rows > 0: # at least one pass was removed/deleted from the table
      flash(f"You have successfully deleted {num_rows} pass")
    else:
      flash("No Passes Were Deleted, The Pass ID Did NOT Match!")
    return redirect("/all")
  else: # GET request --> add a form for confirmation
