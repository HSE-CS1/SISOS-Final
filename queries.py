...
def delete_pass(id):
  # this function will remove a pass from the database
...
def get_student_id(id):
  # this function will return a students id from a row/pass id
  sql = "SELECT student_id FROM passes WHERE id = ?"
  list_of_rows = db.execute(sql, id)
  if len(list_of_rows) == 0: # no matching rows
    return None
  else: # at least one match/row was found (should only be 1)
    # return just the student id number only.
    return list_of_rows[0].get("student_id")
